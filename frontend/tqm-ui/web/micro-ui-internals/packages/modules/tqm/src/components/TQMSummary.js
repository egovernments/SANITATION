...
const TQMSummary = () => {
  ...
  useEffect(() => {
    ...
    const checkValue = localStorage.getItem(
      '/tqm-ui/employee/tqm/search-test-results'
    );
  ...
  const config = {
    select: (data) => ({
        ...
        data?.documents?.[0]?.value !== null
          ? {
              sections: [
                {
                  cardHeader: {
                    value: t('ES_TQM_DOCUMENTS_HEADING'),
                    inlineStyles: isMobile ? {} : { marginTop: 0 },
                  },
                  type: 'COMPONENT',
      ...
      isWorkflowComplete:
        data?.workflowStatus?.processInstances?.[0]?.state?.isTerminateState,
    }),
  };
  ...
  const {
    ...
    revalidate,
    isFetching,
  } = Digit.Hooks.tqm.useViewTestSummary({ tenantId, t, id: id, config });
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      if (respo?.filestoreIds?.[0]) {
        const pdfDownload = await Digit.UploadServices.Filefetch(
          respo?.filestoreIds,
          tenantId
        );
