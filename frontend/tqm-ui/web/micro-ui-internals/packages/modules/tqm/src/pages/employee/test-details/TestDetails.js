...
function TestDetails() {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = Digit.Hooks.tqm.useViewTestResults({
    t,
    id: id,
    tenantId: tenantId,
    config: {
                  ...
                  sections: [
                    {
                      type: 'DATA',
                      cardHeader: {
                        value: t('ES_TQM_TEST_DETAILS_HEADING'),
  ...
  let {
    ...
    isError: isWFError,
    data: WFData,
  } = Digit.Hooks.useWorkflowDetailsFSM({
    tenantId: tenantId,
    id: id,
  ...
  useEffect(() => {
    if (WFData && !isWFLoading) {
      ...
      WFData?.actionState?.applicationStatus ===
      UICustomizations?.workflowStatusMap?.submit
        ? setNextAction(null)
    ...
    mutate(data, {
      onError: (error, variables) => {
        ...
        if (error?.message?.includes('code is not present in mdms')) {
          setShowToast({
            key: 'error',
            action: 'MDMS_INVALID_ERROR_UPDATE_TEST',
          });
          ...
          setShowToast({
            ...
            action:
              nextAction === 'SCHEDULED'
                ? 'ES_TQM_SCHEDULE_UPDATED_FAILED'
                : 'ES_TQM_TEST_UPDATED_FAILED',
          });
          setTimeout(closeToast, 5000);
        }
      },
      onSuccess: (data, variables) => {
        setShowToast({
          key: 'success',
          action: 'ES_TQM_STATUS_UPDATED_SUCCESSFULLY',
        });
        setTimeout(closeToast, 5000);
        ...
        if (
          WFData?.actionState?.applicationStatus ===
          UICustomizations?.workflowStatusMap?.pendingResults
        ) {
          return history.push(
            `/${
              window.contextPath
            }/employee/tqm/response?testId=${id}&isSuccess=${true}`,
            {
  ...
  return (
    <>
      {!isLoading && (
        <ViewComposer data={testData?.data} isLoading={isLoading} />
      )}
      {testData &&
        !isLoading &&
        workflowDetails &&
