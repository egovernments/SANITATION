...
const Create = () => {
  ...
  const CreateAdhocTestSession = Digit.Hooks.useSessionStorage(
    'CREATE_ADHOC_TEST',
    {}
  );
  const [
  ...
  const onSubmit = async (data) => {
    ...
    let qualityCriteria = sessionStorage
      .getItem('Digit.qualityCriteria')
      ?.split(',');
    qualityCriteria.push('document');
    data.QualityParameter = filterObjectKeys(
    ...
    await mutate(modifiedData, {
      ...
      onSuccess: async (data) => {
        ...
        setTimeout(() => {
          closeToast();
          history.push(
            `/tqm-ui/employee/tqm/view-test-results?id=${data.tests[0].testId}&type=adhoc`
          );
        }, 5000);
      },
    });
