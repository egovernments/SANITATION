...
const getTimeFormat = (epochTime) => {
  ...
  return (
    Format.toString() + ":" + epochTime.toString().split(":")[1] + " " + Period
  );
};
...
const getEventSLA = (item) => {
  const days =
    (Date.now() - item.auditDetails.lastModifiedTime) / (1000 * 60 * 60 * 24);
  let time;
  let unit;
...
const filterAllEvents = async (data, variant) => {
  ...
  for (const e of filteredEvents) {
    const actionDownloadLinks =
      e?.eventDetails?.documents?.length > 0 && e?.tenantId
        ? await fetchImageLinksFromFilestoreIds(
            e?.eventDetails?.documents,
            e?.tenantId
          )
        : [];
    ...
    events.push({
      ...
      header:
        e?.eventType === "SYSTEMGENERATED"
          ? getTransformedLocale(e?.name)
          : e?.name,
      eventType: e?.eventType,
      actions: [
        ...(e?.actions?.actionUrls ? e?.actions?.actionUrls : []),
        ...actionDownloadLinks,
      ],
...
const getEventsData = async (variant, tenantId) => {
  ...
  const data = await Digit.EventsServices.Search({
    ...
    auth: !!isLoggedIn,
    ...(variant === "events"
      ? { filter: { eventTypes: "EVENTSONGROUND" } }
      : {}),
  });
