...
const combineResponse = (applications, workflowData, totalCount) => {
  ...
  return applications.map((application) => ({
    ...application,
    assignee:
      workflowInstances[application?.applicationNumber]?.assignes?.[0]?.name,
...
const useEmpBPAREGSearch = (tenantId, filters, params, config = {}) => {
  return useQuery(
    ["BPA_REG_WORK_SEARCH", tenantId, filters, params],
    async () => {
      ...
      const workflowRes = await Digit.WorkflowService.getAllApplication(
        Digit.ULBService.getStateId(),
        { businessIds: businessIds.join() }
      );
      return combineResponse(
        response?.Licenses,
        workflowRes?.ProcessInstances,
        response?.Count
      );
    },
    config
