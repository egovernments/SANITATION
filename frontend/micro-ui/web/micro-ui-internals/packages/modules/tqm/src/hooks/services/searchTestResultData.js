export const searchTestResultData = async ({ t, id, type, tenantId }) => {
  ...
  testcriteraData?.forEach((testItem) => {
    const matchingMdmsItem = mdmsCriteriaData?.mdms?.find(
      (mdmsItem) => mdmsItem.uniqueIdentifier === testItem.criteriaCode
    );
    if (matchingMdmsItem) {
      const mergedData = {
  ...
  const workflowData = await Digit.WorkflowService.getDetailsByIdWorks({
    tenantId,
    id,
    moduleCode: "PQM",
  });
  ...
  if (type !== "adhoc") {
    try {
      ...
      sla = targetTimestamp
        ? Math.ceil(remainingSLA / (24 * 60 * 60 * 1000))
        : 0;
    } catch (err) {
  ...
  const workflowActionMap =
    Digit?.Customizations?.commonUiConfig?.workflowActionMap;
  const sampleProcess = workflowData?.processInstances?.find(
    (i) => i.action === workflowActionMap.submit
  );
  const testProcess = workflowData?.processInstances?.find(
    (i) => i.action === workflowActionMap.update
  );
      ...
      userRoles.includes("PQM_ADMIN") && type !== "adhoc"
        ? [
            ...
            },
            {
              ...
              value: testResponse?.plantCode
                ? t(
                    Digit.Utils.locale.getTransformedLocale(
                      `PQM.Plant_${testResponse?.plantCode}`
                    )
                  )
                : t("ES_TQM_TBD"),
            },
            {
              ...
              value: testResponse?.processCode
                ? t(
                    Digit.Utils.locale.getTransformedLocale(
                      `PQM.Process_${testResponse?.processCode}`
                    )
                  )
                : t("ES_TQM_TBD"),
            },
            {
              ...
              value: testResponse?.stageCode
                ? t(
                    Digit.Utils.locale.getTransformedLocale(
                      `PQM.Stage_${testResponse?.stageCode}`
                    )
                  )
                : t("ES_TQM_TBD"),
            },
            {
              ...
              value: testResponse?.testType
                ? t(
                    Digit.Utils.locale.getTransformedLocale(
                      `PQM.TestType_${testResponse?.testType}`
                    )
                  )
                : t("ES_TQM_TBD"),
            },
            {
              ...
              value: testResponse?.materialCode
                ? t(
                    Digit.Utils.locale.getTransformedLocale(
                      `PQM.Material_${testResponse?.materialCode}`
                    )
                  )
                : t("ES_TQM_TBD"),
            },
            {
              ...
              value: testResponse?.labAssignedTo
                ? t(
                    Digit.Utils.locale.getTransformedLocale(
                      `PQM.QualityTestLab_${testResponse?.labAssignedTo}`
                    )
                  )
                : t("ES_TQM_TBD"),
            },
            {
              ...
              value:
                (testResponse?.scheduledDate &&
                  `${new Date(testResponse?.scheduledDate).getDate()}/${
                    new Date(testResponse?.scheduledDate).getMonth() + 1
                  }/${new Date(testResponse?.scheduledDate).getFullYear()}`) ||
                t("ES_TQM_TBD"),
            },
            {
              key: t("ES_TQM_LABEL_STATUS"),
              value:
                t(`TQM_TEST_STATUS_${testResponse?.wfStatus}`) ||
                t("ES_TQM_TBD"),
            },
            {
              ...
              value:
                sampleProcess && sampleProcess?.auditDetails?.lastModifiedTime
                  ? `${new Date(
                      sampleProcess &&
                        sampleProcess?.auditDetails?.lastModifiedTime
                    ).getDate()}/${
                      new Date(
                        sampleProcess &&
                          sampleProcess?.auditDetails?.lastModifiedTime
            ...
            },
            {
              ...
              value:
                workflowData?.processInstances?.[0]?.state?.isTerminateState ===
                  true &&
                workflowData?.processInstances?.[0]?.auditDetails
                  ?.lastModifiedTime
                  ? `${new Date(
                      workflowData?.processInstances?.[0]?.auditDetails?.lastModifiedTime
                    ).getDate()}/${
                      new Date(
                        workflowData?.processInstances?.[0]?.auditDetails?.lastModifiedTime
                      ).getMonth() + 1
    ...
    testSummary:
      combinedData.length !== 0
        ? [
            ...
            testResponse.status === "PASS"
              ? t("ES_TQM_LABEL_RESULT_PASS")
              : testResponse.status === "FAIL"
              ? t("ES_TQM_LABEL_RESULT_FAIL")
              : t("TQM_TEST_STATUS_PENDING"),
          ]
        : null,
