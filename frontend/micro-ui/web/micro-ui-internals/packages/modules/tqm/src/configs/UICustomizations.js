...
export const UICustomizations = {
  ...
  TqmInboxConfig: {
    preProcess: (data, additionalDetails) => {
      ...
      if (
        Digit.SessionStorage.get("user_plants")?.filter((row) => row.plantCode)
          ?.length === 0 ||
      ...
      const { processCodes, materialCodes, status, dateRange } =
        data?.state?.searchForm || {};
      const { sortOrder } = data?.state?.filterForm || {};
      const { limit, offset } = data?.state?.tableForm || {};
      //processcodes
      data.body.inbox.moduleSearchCriteria.processCodes = processCodes?.map(
        (processCode) => processCode.code
      );

      //materialcodes
      data.body.inbox.moduleSearchCriteria.materialCodes = materialCodes?.map(
        (materialCode) => materialCode.code
      );

      //status
      data.body.inbox.moduleSearchCriteria.wfStatus = status?.map(
        (status) => status.applicationStatus
      );

      //fromDate and toDate
      const { fromDate, toDate } =
        Digit.Utils.tqm.convertDateRangeToEpochObj(dateRange) || {};
      ...
      const activePlantCode = Digit.SessionStorage.get("active_plant")
        ?.plantCode
        ...
        config: {
          ...
          select: (data) => {
            const wfStates = data?.BusinessServices?.[0]?.states
              ?.filter(
                (state) => state.applicationStatus && !state.isTerminateState
              )
              ?.map((state) => {
                return {
  ...
  TqmInboxConfigUlbAdmin: {
    ...
    preProcess: (data, additionalDetails) => {
      ...
      const { id, plantCodes: selectedPlantCodes } =
        data?.state?.searchForm || {};
      const { processCodes, stage, materialCodes, status } =
        data?.state?.filterForm || {};
      const { limit, offset } = data?.state?.tableForm || {};
      //ids
      data.body.inbox.moduleSearchCriteria.testIds = id ? [id] : null;

      data.body.inbox.moduleSearchCriteria.plantCodes = Digit.SessionStorage.get(
        "user_plants"
      )
        ?.filter((row) => row?.plantCode)
        ?.map((row) => row?.plantCode);
      ...
      if (selectedPlantCodes?.length > 0) {
        data.body.inbox.moduleSearchCriteria.plantCodes = selectedPlantCodes
          ?.filter((row) => row?.plantCode)
          ?.map((row) => row?.plantCode);
      }
      //stage
      data.body.inbox.moduleSearchCriteria.stageCodes = stage?.map(
        (st) => st.code
      );

      //materialcodes
      data.body.inbox.moduleSearchCriteria.materialCodes =
        materialCodes?.length > 0 ? materialCodes?.map((row) => row?.code) : [];

      //processcodes
      data.body.inbox.moduleSearchCriteria.processCodes =
        processCodes?.length > 0 ? processCodes?.map((row) => row?.code) : [];
  ...
  SearchTestResults: {
    preProcess: (data, additionalDetails) => {
      ...
      const { processCodes, materialCodes, testType, dateRange } =
        data?.state?.searchForm || {};
      const { sortOrder } = data?.state?.filterForm || {};
      const { limit, offset } = data?.state?.tableForm || {};
      ...
      //processcodes
      data.body.testSearchCriteria.processCodes = processCodes?.map(
        (processCode) => processCode.code
      );

      //materialcodes
      data.body.testSearchCriteria.materialCodes = materialCodes?.map(
        (materialCode) => materialCode.code
      );
      //testType
      data.body.testSearchCriteria.testType = testType?.map(
      ...
      if (
        data?.body?.testSearchCriteria?.testType?.length === 0 ||
        !data?.body?.testSearchCriteria?.testType
      ) {
        data.body.testSearchCriteria.testType = [
          "LAB_SCHEDULED",
          "IOT_SCHEDULED",
  ...
  SearchTestResultsUlbAdmin: {
    preProcess: (data, additionalDetails) => {
      ...
      data.body.testSearchCriteria.plantCodes = Digit.SessionStorage.get(
        "user_plants"
      )
        ?.filter((row) => row?.plantCode)
        ?.map((row) => row?.plantCode);
      //plantCodes
      if (selectedPlantCodes?.length > 0) {
        data.body.testSearchCriteria.plantCodes = selectedPlantCodes
          ?.filter((row) => row?.plantCode)
          ?.map((row) => row?.plantCode);
      }
      data.body.testSearchCriteria.wfStatus = ["SUBMITTED"];
    ...
    onCardClick: (obj, row) => {
      return `view-test-results?tenantId=${obj?.apiResponse?.tenantId}&id=${
        obj?.apiResponse?.testId
      }&from=TQM_BREAD_PAST_TESTS&type=${
        obj?.apiResponse?.testType === "LAB_ADHOC" ? "adhoc" : ""
      }`;
    },
          ...
          return (
            <span className="link">
              <Link
                to={`/${
                  window.contextPath
                }/employee/tqm/view-test-results?tenantId=${Digit.ULBService.getCurrentTenantId()}&id=${value}&from=TQM_BREAD_PAST_TESTS&type=${
                  row?.testType === "LAB_ADHOC" ? "adhoc" : ""
                }`}
              >
                {String(
                  value
  ...
  SensorConfig: {
    preProcess: (data, additionalDetails) => {
      ...
      //status
      data.body.inbox.moduleSearchCriteria.status = status?.map(
        (status) => status.applicationStatus
      );
