...
export const MyBills = ({ stateCode }) => {
  ...
  const { tenantId } =
    ...
    location?.state || { tenantId: _tenantId } ||
    {};

  if (!tenantId && !location?.state?.fromSearchResults) {
    history.replace(`/sanitation-ui/citizen/login`, { from: url });
  }
  ...
  const getPaymentRestrictionDetails = () => {
    ...
    let updatedBussinessService =
      ...
      isDisoconnectFlow === "true"
        ? "DISCONNECT"
        : businessService;
    if (payRestrictiondetails?.length)
      return (
        payRestrictiondetails.filter(
          (e) => e.code == updatedBussinessService
        )?.[0] || {
