...
export const SelectPaymentType = (props) => {
  ...
  const tenantId =
    state?.tenantId || __tenantId || Digit.ULBService.getCurrentTenantId();
  const stateTenant = Digit.ULBService.getStateId();
  const { control, handleSubmit } = useForm();
  const { data: menu, isLoading } = Digit.Hooks.useCommonMDMS(
    stateTenant,
    "DIGIT-UI",
    "PaymentGateway"
  );
  ...
  if (window.location.href.includes("ISWSCON") || wrkflow === "WNS")
    consumerCode = decodeURIComponent(consumerCode);
  if (wrkflow === "WNS")
    consumerCode = stringReplaceAll(consumerCode, "+", "/");
  useEffect(() => {
  ...
  const onSubmit = async (d) => {
        ...
        callbackUrl:
          window.location.href.includes("mcollect") || wrkflow === "WNS"
            ? `${window.location.protocol}//${
              ...
              }/sanitation-ui/citizen/payment/success/${businessService}/${
                wrkflow === "WNS"
                  ? encodeURIComponent(consumerCode)
                  : consumerCode
              }/${tenantId}?workflow=${
    ...
    try {
      const data = await Digit.PaymentService.createCitizenReciept(
        billDetails?.tenantId,
        filterData
      );
  ...
  if (authorization === "true" && !userInfo.access_token) {
    ...
    window.location.href = `/sanitation-ui/citizen/login?from=${encodeURIComponent(
      pathname + search
    )}`;
  }
          ...
          <div className="payment-amount-info" style={{ marginBottom: "26px" }}>
            ...
            <CardSectionHeader>
              ...
              {paymentAmount !== undefined
                ...
                : Number(billDetails?.totalAmount).toFixed(2)}
            </CardSectionHeader>
          </div>
