...
const SelectPaymentType = (props) => {
  ...
  const optionSecound = {
    ...
    name:
      "I am making the payment on behalf of the owner/ consumer of the service",
  };
  ...
  const tenantId =
    state?.tenantId || _tenantId || Digit.UserService.getUser().info?.tenantId;
  ...
  const billDetails =
    bill?.billDetails?.sort((a, b) => b.fromPeriod - a.fromPeriod)?.[0] || [];
  const Arrears =
    bill?.billDetails
  ...
  const onChangePayersName = (value) => {
    setPayersName(value);
    value.length !== 0 &&
    mobileNumberError != "CORE_COMMON_PHONENO_INVALIDMSG" &&
    payersName != ""
      ? setCanSubmit(true)
      : setCanSubmit(false);
  };
  ...
  const onSubmit = () => {
    if (wrkflow === "WNS") {
      history.push(
        `/sanitation-ui/citizen/payment/collect/${businessService}/${consumerCode}?workflow=WNS&consumerCode=${stringReplaceAll(
          consumerCode,
          "+",
          "/"
        )}`,
      ...
      history.push(
        `/sanitation-ui/citizen/payment/collect/${businessService}/${consumerCode}`,
        {
          paymentAmount: paymentAmt,
          tenantId: billDetails.tenantId,
          name:
