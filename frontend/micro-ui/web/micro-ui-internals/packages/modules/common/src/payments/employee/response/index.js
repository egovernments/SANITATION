...
export const SuccessfulPayment = (props) => {
  ...
  const isFSMResponse = location?.pathname?.includes(
    "payment/success/FSM.TRIP_CHARGES"
  );
  const combineResponseFSM = isFSMResponse
    ? `${t("PAYMENT_COLLECT_LABEL")} / ${t("PAYMENT_COLLECT")}`
    : t("PAYMENT_LOCALIZATION_RESPONSE");
  ...
  useEffect(() => {
    switch (selectedAction) {
      case "GO_TO_HOME":
        return history.push("/sanitation-ui/employee");
      case "ASSIGN_TO_DSO":
        return history.push(
          `/sanitation-ui/employee/fsm/application-details/${consumerCode}`
        );
      default:
        return null;
    }
  ...
  const getCardText = () => {
    if (businessService?.includes("BPA")) {
      ...
      let parsedArchitectName = nameOfAchitect
        ...
        : "ARCHITECT";
      return t(
        `ES_PAYMENT_${businessService}_${parsedArchitectName}_SUCCESSFUL_DESCRIPTION`
      );
    } else if (
      ...
      return t(
        `ES_PAYMENT_WS_${businessService?.replace(
          /\./g,
          "_"
        )}_SUCCESSFUL_DESCRIPTION`
      );
    } else {
  ...
  const getPermitOccupancyOrderSearch = async (order, mode = "download") => {
    ...
    if (responseStatus === 201 || responseStatus === 200) {
      mode == "print"
        ? printPdf(
            new Blob([edcrResponseData.data], { type: "application/pdf" })
          )
        : downloadPdf(
            new Blob([edcrResponseData.data], { type: "application/pdf" }),
            `edcrReport.pdf`
  ...
  return (
    <React.Fragment>
        ...
        {generatePdfKey ? (
          <div style={{ display: "flex" }}>
            ...
            (data?.[0]?.status === "APPROVED" ||
              data?.[0]?.status === "PENDING_SANC_FEE_PAYMENT") ? (
              <div
                className="primary-label-btn d-grid"
                style={{ marginLeft: "unset" }}
                onClick={(e) =>
            ...
            {data?.[0]?.businessService === "BPA_LOW" ? (
              <div
                ...
                style={{ marginLeft: "unset" }}
                onClick={(r) =>
                  getPermitOccupancyOrderSearch("buildingpermit-low")
                }
              >
                <DownloadPrefixIcon />
                {t("BPA_PERMIT_ORDER")}
      ...
      {checkFSMResponse ? (
        ...
        >
          <Link to="/sanitation-ui/employee">
