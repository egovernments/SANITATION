...
const WrapPaymentComponent = (props) => {
  ...
  const {
    ...
    error: bpaerror,
  } = Digit.Hooks.obps.useOBPSSearch(
    "",
    {},
    tenantId,
  ...
  const { isLoading, data, isError } = Digit.Hooks.usePaymentUpdate(
    { egId },
    business_service,
    {
      retry: false,
  ...
  const { label } = Digit.Hooks.useApplicationsForBusinessServiceSearch(
    { businessService: business_service },
    { enabled: false }
  );
  ...
  const {
  ...
  } = Digit.Hooks.useRecieptSearch(
    {
      ...
      businessService: business_service,
      receiptNumbers:
        data?.payments?.Payments?.[0]?.paymentDetails[0].receiptNumber,
    },
    {
      retry: false,
      staleTime: Infinity,
  ...
  const { data: generatePdfKey } = Digit.Hooks.useCommonMDMS(
    tenantId,
    "common-masters",
    "ReceiptKey",
    {
      select: (data) =>
        data["common-masters"]?.uiCommonPay?.filter(({ code }) =>
          business_service?.includes(code)
        )[0]?.receiptKey || "consolidatedreceipt",
      retry: false,
      staleTime: Infinity,
      refetchOnWindowFocus: false,
    }
        ...
        {business_service !== "PT" ? (
          ...
          <React.Fragment>
            <Link
              to={
                (applicationNo &&
                  `/sanitation-ui/citizen/payment/my-bills/${business_service}/${applicationNo}`) ||
                "/sanitation-ui/citizen"
              }
            >
  ...
  const printCertificate = async () => {
    ...
    if (applicationDetails) {
      let response = await Digit.PaymentService.generatePdf(
        state,
        { Licenses: applicationDetails?.Licenses },
        generatePdfKeyForTL
      );
  ...
  const getPermitOccupancyOrderSearch = async (order, mode = "download") => {
    ...
    let bpaResponse = await Digit.OBPSService.BPASearch(
      bpaData?.[0]?.tenantId,
      queryObj
    );
    const edcrResponse = await Digit.OBPSService.scrutinyDetails(
      bpaData?.[0]?.tenantId,
      { edcrNumber: bpaData?.[0]?.edcrNumber }
    );
    ...
    reqData["applicationType"] =
      bpaDataDetails?.additionalDetails?.applicationType;
    ...
    if (responseStatus === 201 || responseStatus === 200) {
      mode == "print"
        ...
        : downloadPdf(
            new Blob([edcrresponse.data], { type: "application/pdf" }),
            `edcrReport.pdf`
          );
    }
  };
  ...
  const getBillingPeriod = (billDetails) => {
    const { taxPeriodFrom, taxPeriodTo, fromPeriod, toPeriod } =
      billDetails || {};
    if (taxPeriodFrom && taxPeriodTo) {
      ...
      if (workflw === "mcollect") {
        let from =
          ...
          " " +
          Digit.Utils.date.monthNames[
            new Date(fromPeriod).getMonth()
          ].toString() +
          " " +
  ...
  if (workflw) {
    ...
    if (
      paymentData?.paymentDetails?.[0]?.businessService &&
      paymentData?.paymentDetails?.[0]?.businessService?.includes("BPA")
    ) {
      let nameOfAchitect = sessionStorage.getItem("BPA_ARCHITECT_NAME");
      let parsedArchitectName = nameOfAchitect
        ? JSON.parse(nameOfAchitect)
        : "ARCHITECT";
      bannerText = `CITIZEN_SUCCESS_${paymentData?.paymentDetails[0]?.businessService.replace(
        /\./g,
        "_"
      )}_${parsedArchitectName}_PAYMENT_MESSAGE`;
    } else if (
      business_service?.includes("WS") ||
      business_service?.includes("SW")
    ) {
      bannerText = t(
        `CITIZEN_SUCCESS_${paymentData?.paymentDetails[0].businessService.replace(
          /\./g,
          "_"
        )}_WS_PAYMENT_MESSAGE`
      );
    ...
    <Card>
      ...
      <StatusTable>
        ...
        {(business_service === "PT" || workflw) && (
          <Row
            ...
            text={getBillingPeriod(
              reciept_data?.paymentDetails[0]?.bill?.billDetails[0]
            )}
          />
        )}
        ...
        {(business_service === "PT" || workflw) && (
          <Row
            ...
            text={`₹ ${
              reciept_data?.paymentDetails?.[0]?.totalDue -
              reciept_data?.paymentDetails?.[0]?.totalAmountPaid
            }`}
          />
        )}
        ...
        <Row
          ...
          label={t(
            ommitRupeeSymbol
              ? "CS_PAYMENT_AMOUNT_PAID_WITHOUT_SYMBOL"
              : "CS_PAYMENT_AMOUNT_PAID"
          )}
        ...
        {(business_service !== "PT" || workflw) && (
          <Row
            ...
            text={
              transactionDate &&
              new Date(transactionDate).toLocaleDateString("in")
            }
          />
        )}
      </StatusTable>
      <div style={{ display: "flex" }}>
        ...
        {business_service == "TL" ? (
          <div
            className="primary-label-btn d-grid"
            style={{ marginLeft: "unset", marginTop: "15px" }}
        ...
        (bpaData?.[0]?.status === "APPROVED" ||
          bpaData?.[0]?.status === "PENDING_SANC_FEE_PAYMENT") ? (
          <div
            className="primary-label-btn d-grid"
            style={{ marginLeft: "unset" }}
            onClick={(e) =>
              getPermitOccupancyOrderSearch("occupancy-certificate")
            }
          >
            <DownloadPrefixIcon />
