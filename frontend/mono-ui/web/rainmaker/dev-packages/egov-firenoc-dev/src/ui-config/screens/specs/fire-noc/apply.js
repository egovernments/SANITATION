...
} from "./applyResource/propertyLocationDetails";
...
const setDocsForEditFlow = async (state, dispatch) => {
  ...
  dispatch(
    prepareFinalObject(
      "FireNocTemp[0].applicationDocuments",
      applicationDocuments
    )
  );
  ...
  edited
    ? {}
    : dispatch(prepareFinalObject("documentsEditFlow", uploadedDocumentObject));
};
...
const screenConfig = {
  ...
  beforeInitScreen: (action, state, dispatch) => {
    ...
    prepareEditFlow(state, dispatch, applicationNumber, tenantId).then(
      (response) => {}
    );
