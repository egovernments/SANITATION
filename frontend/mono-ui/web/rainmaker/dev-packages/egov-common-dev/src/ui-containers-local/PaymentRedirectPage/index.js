  ...
  componentDidMount = async () => {
    ...
    try {
      ...
      if (get(pgUpdateResponse, "Transaction[0].txnStatus") === "FAILURE") {
        const url = `/egov-common/acknowledgement?status=${"failure"}&consumerCode=${consumerCode}&tenantId=${tenantId}&businessService=${get(
          pgUpdateResponse,
          "Transaction[0].module",
          localStorage.getItem("pay-businessService")
        )}`;
        ...
        this.getBusinessServiceMdmsData(tenantId).then((response) => {
          const commonPayDetails = get(
            reduxObj,
            "businessServiceMdmsData.common-masters.uiCommonPay"
          );
          const index =
            commonPayDetails &&
          ...
          if (index > -1) {
            ...
            const details = commonPayDetails.filter(
