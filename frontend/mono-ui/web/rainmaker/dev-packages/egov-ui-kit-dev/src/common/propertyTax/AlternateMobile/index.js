...
const VerifyButton = (type, openDialog) => {
  switch (type) {
    ...
    case "LINKNUM":
      return (
        <div className="text-alter-link">
          <Label label="PT_SEC_LINK_NO_TEXT" fontSize="16px" labelStyle={{ color: "#FE7A51", fontWeight: "400" }} />
          <button type="button" className={"button-alter-link"} onClick={() => openDialog()}>
            <Label label="PT_SEC_LINK_NO_BTN"></Label>
          </button>
        </div>
      );
    default:
...
export default class AlternateMobile extends React.Component {
  ...
  getMDMS = async () => {
    if (window && window.mdmsResponse && window.mdmsResponse["doc"]) {
      ...
      let mdmsResp = await httpRequest(`mdms-v2/v1/_search`, "search", [], {
        MdmsCriteria: {
  ...
  loadProperty = async () => {
    ...
    if (propertyId !== "" && tenantId !== "") {
      ...
      this.setState({
        ...
        showWarning: !owners.some((owner) => owner.alternatemobilenumber && owner.status == "ACTIVE"),
      });
    }
  };
