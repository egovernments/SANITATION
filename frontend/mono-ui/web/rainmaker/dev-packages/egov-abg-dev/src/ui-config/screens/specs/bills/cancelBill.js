...
const getData = async (action, state, dispatch) => {
  ...
  try {
    ...
    if (payload) {
      dispatch(
        prepareFinalObject(
          "applyScreenMdmsData.reasonForBillCancel",
          get(payload, "MdmsRes.common-masters.CancelCurrentBillReasons", [])
        )
      );
    }
  } catch (e) {}
...
const screenConfig = {
  ...
  beforeInitScreen: (action, state, dispatch) => {
    ...
    set(
      action.screenConfig,
      "components.div.children.cancelBillDetailsCard.children.cardContent.children.searchContainer.children.reason.props.value",
      get(
        state.screenConfiguration.preparedFinalObject,
        "UpdateBillCriteria.additionalDetails.reason",
        ""
      )
    );
    set(
      action.screenConfig,
      "components.div.children.cancelBillDetailsCard.children.cardContent.children.searchContainer.children.addtionalPenalty.props.value",
      get(
        state.screenConfiguration.preparedFinalObject,
        "UpdateBillCriteria.additionalDetails.additionalPenalty",
        ""
      )
    );
    const additionalDetailsJson =
      "components.div.children.cancelBillDetailsCard.children.cardContent.children.searchContainer.children.addtionalDetails";
    if (
      get(
        state.screenConfiguration.preparedFinalObject,
        "UpdateBillCriteria.additionalDetails.reason",
    ...
    set(
      ...
      get(
        state.screenConfiguration.preparedFinalObject,
        "UpdateBillCriteria.additionalDetails.additionalDetails",
        ""
      )
    );
