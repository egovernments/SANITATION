...
const setSearchResponse = async (
  ...
  if (latitude && longitude) {
    ...
    dispatch(
      prepareFinalObject("BPA.landInfo.address.geoLocation.latitude", latitude)
    );
    dispatch(
      prepareFinalObject(
    ...
    tradeTypes.forEach((type) => {
      if (type.code.split(".")[0] === value) {
        if (type.restrictions) {
          if (
            plotArea <= type.restrictions.maxPlotArea &&
...
const setTaskStatus = async (
  ...
  if (payload && payload.ProcessInstances.length > 0) {
    processInstances = orderWfProcessInstances(payload.ProcessInstances);
    dispatch(
      prepareFinalObject("BPAs.taskStatusProcessInstances", processInstances)
    );
...
export const getMohallaDetails = async (state, dispatch, tenantId) => {
  try {
    ...
    dispatch(prepareFinalObject("mohalla.tenant.localities", mohallaData));
    dispatch(
      handleField(
        "apply",
