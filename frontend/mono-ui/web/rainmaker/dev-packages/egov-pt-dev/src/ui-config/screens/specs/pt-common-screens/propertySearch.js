...
const getMDMSData = async (dispatch) => {
  ...
  try {
    ...
    ptWorkflowDetails.forEach((data) => {
      if (data.enable) {
        dispatch(
          prepareFinalObject("applyScreenMdmsData.isCheckFromWNS", false)
        );
        if (data.businessService.includes("WNS")) {
          dispatch(
            prepareFinalObject("applyScreenMdmsData.isCheckFromWNS", true)
          );
        }
      }
    });
            ...
            newApplicationButton: {
              ...
              onClickDefination: {
                ...
                callBack: () => {
                  ...
                  let applicationNo = getQueryArg(
                    window.location.href,
                    "applicationNumber"
                  );
                  const connectionNo = getQueryArg(
                    window.location.href,
                    "connectionNumber"
                  );
                  const actionType = getQueryArg(
                    window.location.href,
                    "action"
                  );
                  const modeaction = getQueryArg(
                    window.location.href,
                    "modeaction"
                  );
                  const mode = getQueryArg(window.location.href, "mode");
                  url = applicationNo
                    ? url + `&applicationNumber=${applicationNo}`
                    : url;
                  url = connectionNo
                    ? url + `&connectionNumber=${connectionNo}`
                    : url;
                  ...
                  store.dispatch(
                    setRoute(
                      `/pt-common-screens/register-property?redirectUrl=${url}`
                    )
                  );
                },
              },
