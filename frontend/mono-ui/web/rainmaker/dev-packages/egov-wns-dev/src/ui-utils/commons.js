...
export const pushTheDocsUploadedToRedux = async (state, dispatch) => {
  ...
  if (reduxDocuments !== null && reduxDocuments !== undefined) {
    ...
    Object.keys(reduxDocuments).forEach(async (key) => {
      if (
        reduxDocuments !== undefined &&
        reduxDocuments[key] !== undefined &&
        reduxDocuments[key].documents !== undefined
      ) {
        ...
        uploadedDocs.forEach((obj) => {
          ...
          Object.keys(docArrayFromFileStore).forEach((resp) => {
            docArrayFromFileStore[resp].forEach((arr) => {
        ...
        let applyScreenObject = findAndReplace(
          get(state.screenConfiguration.preparedFinalObject, "applyScreen", {}),
          "NA",
          null
        );
...
export const getPropertyObj = async (
  waterConnection,
  locality,
  tenantId,
  isFromSearch
  ...
  for (var i = 0; i < waterConnection.length; i++) {
    if (
      waterConnection[i].propertyId &&
      waterConnection[i].propertyId !== null &&
      waterConnection[i].propertyId !== "NA"
    ) {
      ...
      if (
        uuidsArray.length % 50 === 0 ||
        (uuidsArray.length > 0 && i === waterConnection.length - 1)
      ) {
        let queryObject1 = [];
        ...
        if (!window.location.href.includes("propertyId") || isFromSearch) {
          ...
          if (payload.Properties.length > 0) {
            for (var j = 0; j < payload.Properties.length; j++) {
              propertyArr[payload.Properties[j].propertyId] =
                payload.Properties[j];
            }
          }
        }
        uuids = "";
    ...
    for (var i = 0; i < waterConnection.length; i++) {
      if (
        ...
        if (propertyArr[waterConnection[i].propertyId]) {
          tempPropertyObj = propertyArr[waterConnection[i].propertyId]
            ? propertyArr[waterConnection[i].propertyId]
            : null;
          waterConnection[i].property = tempPropertyObj;
          waterConnection[i].tenantId =
            tempPropertyObj && tempPropertyObj.tenantId
              ? tempPropertyObj.tenantId
              : null;
  ...
  if (get(waterConnection[0], "property.owners")) {
    waterConnection[0].property.owners =
      waterConnection[0].property.owners.filter(
        (owner) => owner.status == "ACTIVE"
      );
  }
  if (
    get(waterConnection[0], "property.units") == "NA" &&
    get(waterConnection[0], "property.additionalDetails") &&
    get(waterConnection[0], "property.additionalDetails.subUsageCategory")
  ) {
    waterConnection[0].property.units = [];
    waterConnection[0].property.units.push({
      usageCategory: get(
        waterConnection[0],
        "property.additionalDetails.subUsageCategory"
      ),
    });
  }
...
export const getSearchResults = async (queryObject, filter = false) => {
  try {
    ...
    if (filter) {
      response.WaterConnection = response.WaterConnection.filter(
        (app) =>
          currentTime > app.dateEffectiveFrom &&
          (app.applicationStatus == "APPROVED" ||
            app.applicationStatus == "CONNECTION_ACTIVATED")
      );
      response.WaterConnection = response.WaterConnection.sort(
        (row1, row2) =>
          row2.auditDetails.createdTime - row1.auditDetails.createdTime
      );
    }

    let result = findAndReplace(response, null, "NA");
    result.WaterConnection[0].waterSourceSubSource =
      result.WaterConnection[0].waterSource.includes("null")
        ? "NA"
        : result.WaterConnection[0].waterSource;
    let waterSource = result.WaterConnection[0].waterSource.includes("null")
      ? "NA"
      : result.WaterConnection[0].waterSource.split(".")[0];
    let waterSubSource = result.WaterConnection[0].waterSource.includes("null")
      ? "NA"
      : result.WaterConnection[0].waterSource.split(".")[1];
...
export const getSearchResultsForSewerage = async (
  queryObject,
  dispatch,
  filter = false
) => {
  ...
  try {
    ...
    let currentTime = new Date().getTime();
    if (filter) {
      response.SewerageConnections = response.SewerageConnections.filter(
        (app) =>
          currentTime > app.dateEffectiveFrom &&
          (app.applicationStatus == "APPROVED" ||
            app.applicationStatus == "CONNECTION_ACTIVATED")
      );
      response.SewerageConnections = response.SewerageConnections.sort(
        (row1, row2) =>
          row2.auditDetails.createdTime - row1.auditDetails.createdTime
      );
    }
    let result = findAndReplace(response, null, "NA");
    result.SewerageConnections = await getPropertyObj(
      result.SewerageConnections
    );
    dispatch(toggleSpinner());
    return result;
  } catch (error) {
    ...
    if (response && response.Properties && response.Properties.length > 0) {
      for (var k = 0; k < response.Properties.length; k++) {
        if (
          ...
          response.Properties[k].owners &&
          response.Properties[k].owners.length > 0
        ) {
          if (
            response.Properties[k].ownershipCategory ==
              "INDIVIDUAL.SINGLEOWNER" ||
            response.Properties[k].ownershipCategory ==
              "INDIVIDUAL.MULTIPLEOWNERS"
          ) {
            for (var i = 0; i < response.Properties[k].owners.length; i++) {
              response.Properties[k].owners[i].correspondenceAddress =
                response.Properties[k].owners[i].permanentAddress;
            }
          }
        }
      }
...
export const validateConnHolderDetails = (holderData) => {
  if (holderData.connectionHolders == null) {
    ...
    holderData.connectionHolders &&
    holderData.connectionHolders.length > 0
  ) {
    let holderOwners = holderData.connectionHolders;
    let valid = [];
    for (let i = 0; i < holderOwners.length; i++) {
      if (
        holderOwners[i].hasOwnProperty("mobileNumber") &&
        holderOwners[i]["mobileNumber"] !== undefined &&
        holderOwners[i]["mobileNumber"] !== "" &&
        holderOwners[i].hasOwnProperty("name") &&
        holderOwners[i]["name"] !== undefined &&
        holderOwners[i]["name"] !== "" &&
        holderOwners[i].hasOwnProperty("fatherOrHusbandName") &&
        holderOwners[i]["fatherOrHusbandName"] !== undefined &&
        holderOwners[i]["fatherOrHusbandName"] !== "" &&
        holderOwners[i].hasOwnProperty("correspondenceAddress") &&
        holderOwners[i]["correspondenceAddress"] !== undefined &&
        holderOwners[i]["correspondenceAddress"] !== "" &&
...
export const handleMandatoryFeildsOfProperty = (applyScreenObject) => {
  ...
  if (
    propertyObject.hasOwnProperty("propertyId") &&
    propertyObject["propertyId"] !== undefined &&
    propertyObject["propertyId"] !== "" &&
    propertyObject.hasOwnProperty("propertyType") &&
    propertyObject["propertyType"] !== undefined &&
    propertyObject["propertyType"] !== "" &&
    propertyObject.hasOwnProperty("usageCategory") &&
    propertyObject["usageCategory"] !== undefined &&
    propertyObject["usageCategory"] !== "" &&
    propertyObject.hasOwnProperty("landArea") &&
    propertyObject["landArea"] !== undefined &&
    propertyObject["landArea"] !== "" &&
    ...
    propertyObject.hasOwnProperty("owners") &&
    propertyObject["owners"] !== undefined &&
    propertyObject["owners"] !== "" &&
...
const handleAddressObjectInProperty = (address) => {
  if (address !== undefined && address !== null && address !== {}) {
    if (
      ...
      address["city"] !== "" &&
      address["city"] !== null &&
      address.hasOwnProperty("doorNo") &&
      address["doorNo"] !== undefined &&
      address["doorNo"] !== "" &&
      address["doorNo"] !== null &&
      address.hasOwnProperty("locality") &&
      address.locality.name !== undefined &&
...
const validatePropertyOwners = (applyScreenObject) => {
  if (applyScreenObject.owners && applyScreenObject.owners.length > 0) {
    ...
    for (let i = 0; i < owners.length; i++) {
      if (
        owners[i].hasOwnProperty("mobileNumber") &&
        owners[i]["mobileNumber"] !== undefined &&
        owners[i]["mobileNumber"] !== "" &&
        owners[i].hasOwnProperty("name") &&
        owners[i]["name"] !== undefined &&
        owners[i]["name"] !== "" &&
        owners[i].hasOwnProperty("fatherOrHusbandName") &&
        owners[i]["fatherOrHusbandName"] !== undefined &&
        owners[i]["fatherOrHusbandName"] !== "" &&
        owners[i].hasOwnProperty("correspondenceAddress") &&
        owners[i]["correspondenceAddress"] !== undefined &&
        owners[i]["correspondenceAddress"] !== ""
      ) {
        valid.push(1);
      } else {
        valid.push(0);
    ...
    if (doc.hasDropdown && doc.dropdownData) {
      ...
      dropdown.menu = dropdown.menu.map((item) => {
        return {
          ...
          name: getTransformedLocale(item.code),
        };
      });
...
const parserFunction = (state) => {
  ...
  let parsedObject = {
    ...
    meterInstallationDate: convertDateToEpoch(
      queryObject.meterInstallationDate
    ),
    connectionExecutionDate: convertDateToEpoch(
      queryObject.connectionExecutionDate
    ),
    ...
    additionalDetails: {
      initialMeterReading:
        ...
        queryObject.additionalDetails.initialMeterReading !== undefined
          ? parseFloat(queryObject.additionalDetails.initialMeterReading)
          : null,
      detailsProvidedBy:
        queryObject.additionalDetails !== undefined &&
...
export const setWSDocuments = async (
  payload,
  sourceJsonPath,
  businessService
) => {
  ...
  if (uploadedDocData !== "NA" && uploadedDocData.length > 0) {
    ...
    const reviewDocData =
      ...
      uploadedDocData.map((item, index) => {
        return {
          title:
            `${businessService}_${item.documentType}`.replace(".", "_") || "",
          link:
            (fileUrlPayload &&
              fileUrlPayload[item.fileStoreId] &&
              getFileUrl(fileUrlPayload[item.fileStoreId])) ||
...
export const downloadAndPrintForNonApply = async (state, dispatch) => {
  let documentPath;
  const { WaterConnection, SewerageConnection } =
    state.screenConfiguration.preparedFinalObject;
  if (
    (WaterConnection.length > 0 && SewerageConnection.length > 0) ||
...
export const prepareDocUploadRedux = async (state, dispatch) => {
  ...
  if (
    ...
    (WaterConnection !== undefined && WaterConnection.length > 0)
  ) {
    documentPath = payload.WaterConnection[0].documents;
    uploadedDocs = await setWSDocuments(
      state.screenConfiguration.preparedFinalObject,
    ...
    uploadedDocs = await setWSDocuments(
      state.screenConfiguration.preparedFinalObject,
      "SewerageConnection[0].documents",
      "WS"
    );
  }
  if (
          ...
          documents: [
            {
              fileName: item.name,
              fileUrl: item.link,
              fileStoreId: documentPath[key].fileStoreId,
            },
          ],
        };
...
export const prefillDocuments = async (payload, destJsonPath, dispatch) => {
  let documentsUploadRedux = {};
  // const uploadedDocData = get(payload, sourceJsonPath);
  let uploadedDocs = await setWSDocuments(
    payload,
    "applyScreen.documents",
    "WS"
  );
  if (
          ...
          documents: [
            {
              fileName: item.name,
              fileUrl: item.link,
              fileStoreId: payload.applyScreen.documents[key].fileStoreId,
            },
          ],
        };
        let splittedString =
          payload.applyScreen.documents[key].documentType.split(".");
        if (splittedString[1] === "ADDRESSPROOF") {
          docUploadRedux[key].dropdown = { value: splittedString.join(".") };
        } else if (splittedString[1] === "IDENTITYPROOF") {
          docUploadRedux[key].dropdown = { value: splittedString.join(".") };
        } else {
          docUploadRedux[key].dropdown = {
            value: payload.applyScreen.documents[key].documentType,
          };
        }
        docUploadRedux[key].documentType =
          payload.applyScreen.documents[key].documentType;
    ...
    var tempDoc = {},
      docType = "";
    var dList = isModifyMode()
      ? payload.applyScreenMdmsData["ws-services-masters"]
    ...
    if (dList !== undefined && dList !== null) {
      ...
      for (var i = 0; i < dList.length; i++) {
        for (var key in docs) {
          ...
          if (
            dList[i].code === docType.substring(0, docType.lastIndexOf("."))
          ) {
            tempDoc[i] = docs[key];
...
export const getDisplayDocFormat = (dataList) => {
  ...
  for (var i = 0; i < dataList.length; i++) {
    if (i == 0) {
      ...
      let getNextDoc = checkValue(
        i,
        tempDoc[i - 1].documentType,
        tempDoc,
        dataList
      );
      if (getNextDoc) {
...
export const applyForWaterOrSewerage = async (state, dispatch) => {
  if (
    get(state, "screenConfiguration.preparedFinalObject.applyScreen.water") &&
...
export const applyForWater = async (state, dispatch) => {
  ...
  let waterId = get(
    state,
    "screenConfiguration.preparedFinalObject.WaterConnection[0].id"
  );
  let method = waterId ? "UPDATE" : "CREATE";
  try {
    const tenantId = get(
      state,
      "screenConfiguration.preparedFinalObject.WaterConnection[0].property.tenantId"
    );
    let response;
    queryObject.tenantId =
      queryObject && queryObject.property && queryObject.property.tenantId
        ? queryObject.property.tenantId
        : null;
    if (method === "UPDATE") {
      ...
      let queryObjectForUpdate = get(
        state,
        "screenConfiguration.preparedFinalObject.WaterConnection[0]"
      );
      let waterSource = get(
        state.screenConfiguration.preparedFinalObject,
        "DynamicMdms.ws-services-masters.waterSource.selectedValues[0].waterSourceType",
        null
      );
      let waterSubSource = get(
        state.screenConfiguration.preparedFinalObject,
        "DynamicMdms.ws-services-masters.waterSource.selectedValues[0].waterSubSource",
        null
      );
      queryObjectForUpdate.waterSource = queryObjectForUpdate.waterSource
        ? queryObjectForUpdate.waterSource
        : waterSource;
      queryObjectForUpdate.waterSubSource = queryObjectForUpdate.waterSubSource
        ? queryObjectForUpdate.waterSubSource
        : waterSubSource;
      ...
      queryObjectForUpdate.additionalDetails.locality =
        queryObjectForUpdate.property.address.locality.code;
      queryObjectForUpdate = findAndReplace(queryObjectForUpdate, "NA", null);
      await httpRequest("post", "/ws-services/wc/_update", "", [], {
        WaterConnection: queryObjectForUpdate,
      });
      let searchQueryObject = [
        { key: "tenantId", value: queryObjectForUpdate.tenantId },
        { key: "applicationNumber", value: queryObjectForUpdate.applicationNo },
      ];
      let searchResponse = await getSearchResults(searchQueryObject);
      dispatch(
        prepareFinalObject("WaterConnection", searchResponse.WaterConnection)
      );
      enableField(
        "apply",
        "components.div.children.footer.children.nextButton",
      ...
      queryObject.additionalDetails.locality =
        queryObject.property.address.locality.code;
      ...
      if (isModifyMode()) {
        set(
          queryObject,
          "waterSource",
          getWaterSource(queryObject.waterSource, queryObject.waterSubSource)
        );
      }
      set(
        queryObject,
        "channel",
        process.env.REACT_APP_NAME === "Citizen" ? "CITIZEN" : "CFC_COUNTER"
      );
      response = await httpRequest("post", "/ws-services/wc/_create", "", [], {
        WaterConnection: queryObject,
      });
      ...
      if (isModifyMode()) {
        response.WaterConnection = await getPropertyObj(
        ...
        dispatch(
          prepareFinalObject("applyScreen", response.WaterConnection[0])
        );
...
export const applyForSewerage = async (state, dispatch) => {
  ...
  let sewerId = get(
    state,
    "screenConfiguration.preparedFinalObject.SewerageConnection[0].id"
  );
  let method = sewerId ? "UPDATE" : "CREATE";
  try {
    const tenantId = get(
      state,
      "screenConfiguration.preparedFinalObject.SewerageConnection[0].property.tenantId"
    );
    let response;
    ...
    if (method === "UPDATE") {
      ...
      let queryObjectForUpdate = get(
        state,
        "screenConfiguration.preparedFinalObject.SewerageConnection[0]"
      );
      queryObjectForUpdate = { ...queryObjectForUpdate, ...queryObject };
      set(queryObjectForUpdate, "processInstance.action", "SUBMIT_APPLICATION");
      ...
      queryObjectForUpdate.additionalDetails.locality =
        queryObjectForUpdate.property.address.locality.code;
      queryObjectForUpdate = findAndReplace(queryObjectForUpdate, "NA", null);
      await httpRequest("post", "/sw-services/swc/_update", "", [], {
        SewerageConnection: queryObjectForUpdate,
      });
      let searchQueryObject = [
        { key: "tenantId", value: queryObjectForUpdate.tenantId },
        { key: "applicationNumber", value: queryObjectForUpdate.applicationNo },
      ];
      let searchResponse = await getSearchResultsForSewerage(
        searchQueryObject,
        dispatch
      );
      dispatch(
        prepareFinalObject(
          "SewerageConnection",
          searchResponse.SewerageConnections
        )
      );
      enableField(
        "apply",
      ...
      response = await httpRequest("post", "/sw-services/swc/_create", "", [], {
        SewerageConnection: queryObject,
      });
      dispatch(
        prepareFinalObject("SewerageConnection", response.SewerageConnections)
      );
      enableField(
        "apply",
        "components.div.children.footer.children.nextButton",
      ...
      if (isModifyMode()) {
        response.SewerageConnections = await getPropertyObj(
          response.SewerageConnections,
          "",
          "",
          true
        ...
        dispatch(
          prepareFinalObject("applyScreen", response.SewerageConnections[0])
        );
        dispatch(prepareFinalObject("modifyAppCreated", true));
      }
      if (!isModifyMode()) {
  ...
  try {
    ...
    queryObject.tenantId =
      queryObject && queryObject.property && queryObject.property.tenantId
        ...
        : null;
    if (method === "UPDATE") {
      let queryObjectForUpdateWater = get(
        state,
        "screenConfiguration.preparedFinalObject.WaterConnection[0]"
      );
      let waterSource = get(
      ...
      queryObjectForUpdateWater.waterSource =
        queryObjectForUpdateWater.waterSource
          ? queryObjectForUpdateWater.waterSource
          : waterSource;
      queryObjectForUpdateWater.waterSubSource =
        queryObjectForUpdateWater.waterSubSource
          ? queryObjectForUpdateWater.waterSubSource
          : waterSubSource;
      let queryObjectForUpdateSewerage = get(
        state,
        "screenConfiguration.preparedFinalObject.SewerageConnection[0]"
      );
      ...
      queryObjectForUpdateWater = findAndReplace(
        queryObjectForUpdateWater,
        "NA",
        null
      );
      ...
      queryObjectForUpdateSewerage = findAndReplace(
        queryObjectForUpdateSewerage,
        "NA",
        null
      );
      ...
      set(
        queryObjectForUpdateSewerage,
        "additionalDetails.appCreatedDate",
        get(
          state.screenConfiguration.preparedFinalObject,
          "SewerageConnection[0].additionalDetails.appCreatedDate"
        )
      ...
      queryObjectForUpdateWater.additionalDetails.locality =
        queryObjectForUpdateWater.property.address.locality.code;
      if (typeof queryObjectForUpdateSewerage.additionalDetails !== "object") {
        queryObjectForUpdateSewerage.additionalDetails = {};
      }
      queryObjectForUpdateSewerage.additionalDetails.locality =
        queryObjectForUpdateSewerage.property.address.locality.code;
      await httpRequest("post", "/ws-services/wc/_update", "", [], {
        WaterConnection: queryObjectForUpdateWater,
      });
      await httpRequest("post", "/sw-services/swc/_update", "", [], {
        SewerageConnection: queryObjectForUpdateSewerage,
      });
      let searchQueryObjectWater = [
        { key: "tenantId", value: queryObjectForUpdateWater.tenantId },
        {
          key: "applicationNumber",
          value: queryObjectForUpdateWater.applicationNo,
        },
      ];
      let searchQueryObjectSewerage = [
        ...
        {
          key: "applicationNumber",
          value: queryObjectForUpdateSewerage.applicationNo,
        },
      ];
      let searchResponse = await getSearchResults(searchQueryObjectWater);
      let sewerageResponse = await getSearchResultsForSewerage(
        searchQueryObjectSewerage,
        dispatch
      );
      dispatch(
        prepareFinalObject("WaterConnection", searchResponse.WaterConnection)
      ...
      dispatch(
        prepareFinalObject(
          "SewerageConnection",
          sewerageResponse.SewerageConnections
        )
      );
      enableField(
        "apply",
      ...
      const sewerageResponse = await httpRequest(
        "post",
...
const setApplicationNumberBox = (state, dispatch) => {
  let applicationNumberWater = get(
    state,
    "screenConfiguration.preparedFinalObject.WaterConnection[0].applicationNo",
    null
  );
  let applicationNumberSewerage = get(
    state,
    "screenConfiguration.preparedFinalObject.SewerageConnection[0].applicationNo",
    null
  );
  if (applicationNumberSewerage && applicationNumberWater) {
    handleApplicationNumberDisplayForBoth(
      dispatch,
...
const handleApplicationNumberDisplayForBoth = (
  dispatch,
  applicationNumberWater,
  applicationNumberSewerage
) => {
...
export const getMdmsDataForMeterStatus = async (dispatch) => {
  ...
  try {
    ...
    let data = payload.MdmsRes["ws-services-calculation"].MeterStatus.map(
      (ele) => {
        return { code: ele };
      }
    );
...
export const getPastPaymentsForWater = async (dispatch) => {
  ...
  try {
    ...
    let uniqueConsumberCodes = consumerCodesList.filter(
    ...
    if (response && response.Payments) {
      ...
      const filteredArray = response.Payments.filter(
        (data) => data.mobileNumber == userNumber
      );
  ...
  switch (service) {
    case serviceConst.WATER:
      try {
        httpRequest(
          "post",
          FETCHCONNECTIONDETAILS.GET.URL,
          FETCHCONNECTIONDETAILS.GET.ACTION,
          receiptQueryString
        ).then(async (payloadReceiptDetails) => {
          const queryStr = [
            { key: "key", value: "ws-consolidatedacknowlegment" },
            { key: "tenantId", value: commonConfig.tenantId },
          ];

          payloadReceiptDetails.WaterConnection = await getPropertyObj(
            payloadReceiptDetails.WaterConnection
          );
          if (
            payloadReceiptDetails.WaterConnection[0].property.additionalDetails
              .isRainwaterHarvesting !== undefined &&
            payloadReceiptDetails.WaterConnection[0].property.additionalDetails
              .isRainwaterHarvesting !== null
          ) {
            if (
              payloadReceiptDetails.WaterConnection[0].property
                .additionalDetails.isRainwaterHarvesting === true
            ) {
              payloadReceiptDetails.WaterConnection[0].property.additionalDetails.isRainwaterHarvesting =
                "SCORE_YES";
            } else {
              payloadReceiptDetails.WaterConnection[0].property.additionalDetails.isRainwaterHarvesting =
                "SCORE_NO";
          ...
          httpRequest(
            "post",
            DOWNLOADCONNECTIONDETAILS.GET.URL,
            DOWNLOADCONNECTIONDETAILS.GET.ACTION,
            queryStr,
            { WaterConnection: payloadReceiptDetails.WaterConnection },
            {
              Accept: commonConfig.singleInstance
    ...
    case serviceConst.SEWERAGE:
      try {
        httpRequest(
          "post",
          FETCHSWCONNECTIONDETAILS.GET.URL,
          FETCHSWCONNECTIONDETAILS.GET.ACTION,
          receiptQueryString
        ).then(async (payloadReceiptDetails) => {
          const queryStr = [
          ...
          payloadReceiptDetails.SewerageConnections = await getPropertyObj(
            payloadReceiptDetails.SewerageConnections
          );
          httpRequest(
            "post",
            DOWNLOADCONNECTIONDETAILS.GET.URL,
            DOWNLOADCONNECTIONDETAILS.GET.ACTION,
            queryStr,
            { SewerageConnections: payloadReceiptDetails.SewerageConnections },
            {
              Accept: commonConfig.singleInstance
                ? "application/pdf,application/json"
                : "application/pdf",
  ...
  try {
    ...
    if (response.SewerageConnections.length > 0) {
      ...
      for (let i = 0; i < response.SewerageConnections.length; i++) {
        response.SewerageConnections[i].service = _.capitalize(
...
export const billingPeriodMDMS = (toPeriod, payloadbillingPeriod, service) => {
  ...
  if (
    ...
    payloadbillingPeriod["ws-services-masters"].billingPeriod.forEach((obj) => {
      if (obj.connectionType === "Metered" && connectionType === "Metered") {
        ...
        obj.connectionType === "Non Metered" &&
  ...
  if (
    service === serviceConst.SEWERAGE &&
    payloadbillingPeriod["sw-services-calculation"] &&
    payloadbillingPeriod["sw-services-calculation"].billingPeriod !==
      undefined &&
    payloadbillingPeriod["sw-services-calculation"].billingPeriod !== null
  ) {
    payloadbillingPeriod["sw-services-calculation"].billingPeriod.forEach(
      (obj) => {
...
export const downloadBill = (receiptQueryString, mode) => {
  ...
  try {
    httpRequest(
      ...
      let data = [];
      payloadReceiptDetails.Bill[0].billDetails.map((curEl) =>
        data.push(curEl)
      );
      ...
      httpRequest(
        ...
        let demandAmount = getDemandBills.Demands[0].demandDetails.reduce(
          (accum, item) => accum + item.taxAmount,
          0
        );
        let partiallyPaid = getDemandBills.Demands[0].demandDetails.reduce(
          (accm, item) => accm + item.collectionAmount,
          0
        );
        if (billTotalAmount <= 0) {
          ...
          payloadReceiptDetails.Bill[0].AdvanceAdjustedValue =
            partiallyPaid > 0 ? partiallyPaid : 0;
        } else {
        ...
        if (billTotalAmount > 0) {
          ...
          if (sortData.length > 0) {
            let totalArrearsAmount = sortData.map(
              (el) => el.amount + totalAmount
            );
            previousArrears = totalArrearsAmount.reduce((a, b) => a + b);
          }
          payloadReceiptDetails.Bill[0].arrearAmount =
            previousArrears.toFixed(2);
        }

        payloadReceiptDetails.Bill[0].billDetails.sort(
        ...
        httpRequest(
          ...
          [],
          requestBody
        ).then((payloadbillingPeriod) => {
          let waterMeteredDemandExipryDate = 0,
            waterNonMeteredDemandExipryDate = 0,
            sewerageNonMeteredDemandExpiryDate = 0;
          const service =
            payloadReceiptDetails.Bill &&
            payloadReceiptDetails.Bill.length > 0 &&
            payloadReceiptDetails.Bill[0].businessService
              ? payloadReceiptDetails.Bill[0].businessService
              : "WS";
          if (
            service === "WS" &&
            ...
            payloadbillingPeriod.MdmsRes[
              ...
              if (
                obj.connectionType === "Metered" &&
                getQueryArg(window.location.href, "connectionType") ===
                  "Metered"
              ) {
                payloadReceiptDetails.Bill[0].billDetails[0]["expiryDate"] =
                  payloadReceiptDetails.Bill[0].billDetails[0].toPeriod +
                  obj.demandExpiryDate;
              } else if (
                obj.connectionType === "Non Metered" &&
                getQueryArg(window.location.href, "connectionType") ===
                  "Non Metered"
          ...
          httpRequest(
            "post",
            DOWNLOADBILL.GET.URL,
            DOWNLOADBILL.GET.ACTION,
            queryStr,
            { Bill: payloadReceiptDetails.Bill },
...
export const findAndReplace = (obj, oldValue, newValue) => {
  Object.keys(obj).forEach((key) => {
    if (obj[key] instanceof Object || obj[key] instanceof Array)
      findAndReplace(obj[key], oldValue, newValue);
...
export const downloadApp = async (wnsConnection, type, mode, dispatch) => {
  ...
  if (wnsConnection[0].service === serviceConst.WATER) {
    // for Estimate api
    if (
      wnsConnection[0].property.rainWaterHarvesting !== undefined &&
      wnsConnection[0].property.rainWaterHarvesting !== null
    ) {
  ...
  try {
    ...
    if (type === "sanctionLetter") {
      ...
      wnsConnection[0].slaDate = connectionExecutionDate.setDate(
        connectionExecutionDate.getDate() + wnsConnection[0].sla
      );
    }

    if (type === "application") {
      if (
        wnsConnection[0].property &&
        wnsConnection[0].property.units &&
        wnsConnection[0].property.units.length > 0 &&
        wnsConnection[0].property.units[0].usageCategory
      ) {
        wnsConnection[0].property.propertySubUsageType =
          wnsConnection[0].property.units[0].usageCategory;
      }
      if (wnsConnection[0].service === serviceConst.WATER) {
        if (
      ...
      if (res && res.filestoreIds && res.filestoreIds.length > 0) {
        res.filestoreIds.map((fileStoreId) => {
          if (type === "sanctionLetter") {
            store.dispatch(
              prepareFinalObject(
                "WaterConnection[0].additionalDetails.sanctionFileStoreId",
                fileStoreId
              )
            );
            ...
            store.dispatch(
              prepareFinalObject(
                "WaterConnection[0].additionalDetails.estimationFileStoreId",
                fileStoreId
              )
            );
          }
...
export const isActiveProperty = (propertyObj) => {
  ...
  let ptWorkflowDetails = get(
    storeData,
    "screenConfiguration.preparedFinalObject.applyScreenMdmsData.PropertyTax.PTWorkflow",
    []
  );
  let status = true;
    ...
    ptWorkflowDetails.forEach((data) => {
      if (data.enable) {
        if (data.businessService.includes("WNS")) {
          if (
            propertyObj.status === "INACTIVE" ||
            propertyObj.status === "INWORKFLOW"
          ) {
            status = false;
          }
        } else {
...
export const getOpenSearchResultsForWater = async (
  queryObject,
  requestBody,
  dispatch
) => {
  try {
    ...
    result.WaterConnection = await getPropertyObj(
      result.WaterConnection,
      locality,
      tenantId
    );
    return result;
...
export const getOpenSearchResultsForSewerage = async (
  queryObject,
  requestBody,
  dispatch
) => {
