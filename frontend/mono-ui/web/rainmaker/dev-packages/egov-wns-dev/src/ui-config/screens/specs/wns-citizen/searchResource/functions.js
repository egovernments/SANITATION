...
export const searchApiCall = async (state, dispatch) => {
  ...
  let queryObject = [{ key: "offset", value: "0" }];
  ...
  if (!(isSearchBoxFirstRowValid && isSearchBoxSecondRowValid)) {
    ...
    (searchScreenObject["propertyId"] === undefined ||
      searchScreenObject["propertyId"] === "") &&
    (searchScreenObject["mobileNumber"] === undefined ||
      searchScreenObject["mobileNumber"] === "") &&
    (searchScreenObject["connectionNumber"] === undefined ||
      searchScreenObject["connectionNumber"] === "") &&
    (searchScreenObject["oldConnectionNumber"] === undefined ||
      searchScreenObject["oldConnectionNumber"] === "")
  ) {
    dispatch(
      toggleSnackbar(true, { labelKey: "ERR_WS_FILL_VALID_FIELDS" }, "warning")
    );
  } else {
    ...
    try {
      ...
      for (let i = 0; i < combinedSearchResults.length; i++) {
        ...
        if (
          element.property &&
          element.property !== "NA" &&
          element.connectionNo !== null &&
          element.connectionNo !== "NA"
        ) {
          ...
          billResults &&
          billResults.Bill &&
          Array.isArray(billResults.Bill) &&
          billResults.Bill.length > 0
            ? billResults.Bill.map((bill) => {
                let updatedDueDate = 0;
                if (element.service === serviceConst.WATER) {
                  updatedDueDate =
                    element.connectionType === "Metered"
                      ? bill.billDetails[0].toPeriod +
                        waterMeteredDemandExipryDate
                      : bill.billDetails[0].toPeriod +
                        waterNonMeteredDemandExipryDate;
                } else if (element.service === serviceConst.SEWERAGE) {
                  updatedDueDate =
                    bill.billDetails[0].toPeriod +
                    sewerageNonMeteredDemandExpiryDate;
                }
                let obj = {
