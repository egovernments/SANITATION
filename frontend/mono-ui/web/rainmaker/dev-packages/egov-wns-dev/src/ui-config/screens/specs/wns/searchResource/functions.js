    ...
    try {
      ...
      for (let i = 0; i < combinedSearchResults.length; i++) {
        ...
        if (
          ...
          appStatus = combinedWFSearchResults.filter((item) =>
            item.businessId.includes(element.applicationNo)
          )[0];
    ...
    element.property.address.tenantId !== null
      ? getLocaleLabels(
          ...
          `TENANT_TENANTS_${getTransformedLocale(
            element.property.address.tenantId
          )}`
        )
      : "";
  let localityName =
    ...
    element.property.address.locality.code !== null
      ? getLocaleLabels(
          "NA",
          `${getTransformedLocale(element.property.address.tenantId)}_REVENUE_${
            element.property.address.locality.code
          }`
        )
      : "";

  return city === "" && localityName === ""
    ? getLocaleLabels("NA", "WS_NA")
    : `${localityName}, ${city}`;
};
