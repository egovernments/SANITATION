...
const getMDMSAppType = async (dispatch) => {
  ...
  try {
    ...
    if (
      payload &&
      payload.MdmsRes["ws-services-masters"] &&
      payload.MdmsRes["ws-services-masters"].ApplicationType !== undefined
    ) {
      payload.MdmsRes["ws-services-masters"].ApplicationType.forEach((obj) =>
        applicationType.push({
          code: obj.code.replace(/_/g, " "),
          name: obj.name,
          businessService: obj.businessService,
        })
      );
      applicationType.forEach((type) =>
        getBusinessService(type.businessService, dispatch)
      );
      dispatch(
        prepareFinalObject(
...
const getBusinessService = async (businessService, dispatch) => {
  ...
  if (
    payload.BusinessServices[0].businessService === "NewWS1" ||
    payload.BusinessServices[0].businessService === "NewSW1"
  ) {
    ...
    if (
      payload.BusinessServices[0].businessService === "ModifyWSConnection" ||
      payload.BusinessServices[0].businessService === "ModifySWConnection"
    ) {
      const applicationStatus = commonGetAppStatus(payload);
      dispatch(
        prepareFinalObject(
...
export const getMdmsTenantsData = async (dispatch) => {
  ...
  try {
    ...
    dispatch(
      prepareFinalObject("applyScreenMdmsData.tenant", payload.MdmsRes.tenant)
    );
  } catch (e) {}
};
...
const employeeSearchResults = {
  ...
  beforeInitScreen: (action, state, dispatch) => {
    ...
    dispatch(
      prepareFinalObject("searchConnection.tenantId", getTenantIdCommon())
    );
