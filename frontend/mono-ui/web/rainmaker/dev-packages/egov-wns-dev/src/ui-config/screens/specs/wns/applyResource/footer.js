...
const setReviewPageRoute = (state, dispatch) => {
  let roadCuttingInfo = get(
    state,
    "screenConfiguration.preparedFinalObject.applyScreen.roadCuttingInfo",
    []
  );
  if (roadCuttingInfo && roadCuttingInfo.length > 0) {
    let formatedRoadCuttingInfo = roadCuttingInfo.filter(
      (value) => value.isEmpty !== true
    );
    dispatch(
      prepareFinalObject("applyScreen.roadCuttingInfo", formatedRoadCuttingInfo)
    );
  }
  let tenantId = getTenantIdCommon();
  const applicationNumber = get(
    state,
    "screenConfiguration.preparedFinalObject.applyScreen.applicationNo"
  );
  const appendUrl =
  ...
  if (
    get(
      state,
      "screenConfiguration.preparedFinalObject.WaterConnection[0].additionalDetails.locality",
      null
    ) === null
  ) {
    dispatch(
      prepareFinalObject(
        "WaterConnection[0].additionalDetails.locality",
        get(
          state,
          "screenConfiguration.preparedFinalObject.WaterConnection[0].property.address.locality.code"
        )
      )
    );
  }
...
const showHideFeilds = (dispatch, value) => {
  let mStep =
    getQueryArg(window.location.href, "mode") &&
    getQueryArg(window.location.href, "mode").toUpperCase() == "MODIFY"
  ...
  try {
    ...
    dispatch(
      prepareFinalObject(
        ...
        payload.MdmsRes["ws-services-masters"].Documents
      )
    );
...
const callBackForNext = async (state, dispatch) => {
  ...
  if (activeStep === 0) {
    ...
    validateFields(
      "components.div.children.formwizardFirstStep.children.IDDetails.children.cardContent.children.propertyID.children",
      state,
      dispatch
    );
    validateFields(
    ...
    if (
      ...
      let application = findAndReplace(
      ...
      if (!isActiveProperty(applyScreenObj.property)) {
        dispatch(
          toggleSnackbar(
            true,
            {
              labelKey: `ERR_WS_PROP_STATUS_${applyScreenObj.property.status}`,
              labelName: `Property Status is ${applyScreenObj.property.status}`,
            },
            "warning"
          )
        );
      ...
      if (waterSourceType == null || waterSourceType == "null") {
        dispatch(
          prepareFinalObject(
            "DynamicMdms.ws-services-masters.waterSource.selectedValues[0].waterSourceTypewaterSource.selectedValues[0].waterSourceType",
            ""
          )
        );
      }
      ...
      if (searchPropertyId !== undefined && searchPropertyId !== "") {
        if (!isActiveProperty(applyScreenObject.property)) {
          dispatch(
            toggleSnackbar(
              true,
              {
                labelKey: `ERR_WS_PROP_STATUS_${applyScreenObject.property.status}`,
                labelName: `Property Status is ${applyScreenObject.property.status}`,
              },
              "warning"
            )
          );
        ...
        if (validateConnHolderDetails(applyScreenObject)) {
          ...
          if (applyScreenObject.water || applyScreenObject.sewerage) {
              ...
              dispatch(
                toggleSnackbar(
                  true,
                  {
                    ...
                    labelName:
                      "No Property records found, please search or create a new property",
                  },
                  "warning"
                )
              );
            ...
            let waterData = get(
              state,
              "screenConfiguration.preparedFinalObject.WaterConnection"
            );
            let sewerData = get(
              state,
              "screenConfiguration.preparedFinalObject.SewerageConnection"
            );
            let waterChecked = get(
              state,
              "screenConfiguration.preparedFinalObject.applyScreen.water"
            );
            let sewerChecked = get(
              state,
              "screenConfiguration.preparedFinalObject.applyScreen.sewerage"
            );
            let modifyAppCreated = get(
              state,
              "screenConfiguration.preparedFinalObject.modifyAppCreated"
            );
            if (isFormValid) {
              if (
                (waterData && waterData.length > 0) ||
                (sewerData && sewerData.length > 0)
              ) {
                if (waterChecked && sewerChecked) {
                  ...
                  (sewerChecked && sewerData.length === 0) ||
                  (isModifyMode() &&
                    sewerData.length === 1 &&
                    !modifyAppCreated)
                ) {
                  ...
                  (waterChecked && waterData.length === 0) ||
                  (isModifyMode() &&
                    waterData.length === 1 &&
                    !modifyAppCreated)
                ) {
                ...
                if (waterData.length === 0 && sewerData.length === 0) {
                  if (!window.location.href.includes("mode=MODIFY&action=edit"))
                    isFormValid = await applyForWaterOrSewerage(
                      state,
                      dispatch
                    );
                }
  ...
  if (activeStep === 1) {
    if (isModifyMode()) {
      let validate = validateFields(
        "components.div.children.formwizardThirdStep.children.additionDetails.children.cardContent.children.modificationsEffectiveFrom.children.cardContent.children.modificationEffectiveDate.children",
        state,
        dispatch
      );
      if (validate) {
      ...
      if (moveToReview(state, dispatch)) {
        ...
        let connType = get(
          state.screenConfiguration.preparedFinalObject,
          "applyScreen.connectionType",
          ""
        );
        let applicationNumber = get(
          state.screenConfiguration.preparedFinalObject,
          "applyScreen.applicationNo",
          ""
        );
        if (
          ...
          if (applicationNumber.includes("WS")) {
            ...
            dispatch(
              handleField(
                "apply",
                "components.div.children.formwizardThirdStep.children.additionDetails.children.cardContent.children.connectiondetailscontainer.children.cardContent.children.connectionDetails.children.dynamicMdmsWaterSource.props.dropdownFields[0]",
                "isRequired",
                false
              )
            );
            ...
            dispatch(
              handleField(
                "apply",
                "components.div.children.formwizardThirdStep.children.additionDetails.children.cardContent.children.connectiondetailscontainer.children.cardContent.children.connectionDetails.children.dynamicMdmsWaterSource.props.dropdownFields[1]",
                "isRequired",
                false
              )
            );
            ...
            dispatch(
              handleField(
                "apply",
                "components.div.children.formwizardThirdStep.children.additionDetails.children.cardContent.children.activationDetailsContainer.children.cardContent.children.activeDetails.children.connectionExecutionDate",
                "required",
                true
              )
            );
          ...
          if (
            connType === undefined ||
            connType === "Non Metered" ||
            connType === "Bulk-supply" ||
            connType !== "Metered"
          ) {
            dispatch(
              handleField(
        ...
        let raodCuttingInfos = get(
          state,
          "screenConfiguration.preparedFinalObject.applyScreen.raodCuttingInfos",
          []
        );
        ...
        if (roadCuttingInfo && roadCuttingInfo.length > 0) {
          dispatch(
            prepareFinalObject(
              "applyScreen.tempRoadCuttingInfo",
              roadCuttingInfo
            )
          );
          let formatedRoadCuttingInfo = roadCuttingInfo.filter(
            (value) => value.emptyObj !== true
          );
          dispatch(
            prepareFinalObject(
              "applyScreen.roadCuttingInfo",
              formatedRoadCuttingInfo
            )
          );
        }
      } else {
  ...
  if (activeStep === 2 && process.env.REACT_APP_NAME !== "Citizen") {
    if (isModifyMode()) {
      ...
      let roadCuttingInfos = get(
        state,
        "screenConfiguration.preparedFinalObject.applyScreen.roadCuttingInfos",
        []
      );
      ...
      let multipleRoadTypeCardPath =
        "components.div.children.formwizardThirdStep.children.additionDetails.children.cardContent.children.roadCuttingChargeContainer.children.cardContent.children.applicantTypeContainer.children.roadCuttingChargeInfoCard.children.multipleApplicantInfo.props.items";
      ...
      if (applicationStatus === "PENDING_FOR_CONNECTION_ACTIVATION") {
        let connectionDetailsCard = validateFieldOfWNS(
        ...
        let activeDetailsCard = validateFields(
      ...
      if (roadCuttingInfo && roadCuttingInfo.length > 0) {
        ...
        dispatch(
          prepareFinalObject("applyScreen.roadCuttingInfos", roadCuttingInfo)
        );
        ...
        // });
        dispatch(
          prepareFinalObject("applyScreen.roadCuttingInfo", filteredInfo)
        );
      }

      if (
        getQueryArg(window.location.href, "action") === "edit" &&
        (!isModifyMode() || (isModifyMode() && isModifyModeAction()))
      ) {
        setReviewPageRoute(state, dispatch);
      }
      isFormValid = true;
    ...
    dispatch(
      handleField(
        "apply",
        "components.div.children.formwizardFourthStep.children.snackbarWarningMessage",
        "props.propertyId",
        get(applyScreenObj, "property.propertyId", "")
      )
    );
    if (isActiveProperty(applyScreenObj.property)) {
      dispatch(
  ...
  if (activeStep === 3) {
    ...
    if (waterId && sewerId) {
      isFormValid = await acknoledgementForBothWaterAndSewerage(
        state,
        activeStep,
        isFormValid,
        dispatch
      ...
      isFormValid = await acknoledgementForWater(
        state,
        activeStep,
        isFormValid,
        dispatch
      ...
      isFormValid = await acknoledgementForSewerage(
        state,
        activeStep,
        isFormValid,
        dispatch
...
const moveToSuccess = (combinedArray, dispatch) => {
  const tenantId =
    get(combinedArray[0].property, "tenantId") ||
    get(combinedArray[0], "tenantId");
  const purpose = "apply";
...
const acknoledgementForBothWaterAndSewerage = async (
  state,
  activeStep,
  isFormValid,
  dispatch
  ...
  if (isFormValid) {
    ...
    if (activeStep === 3) {
      ...
      let WaterConnection = get(
        state.screenConfiguration.preparedFinalObject,
        "WaterConnection"
      );
      let SewerageConnection = get(
        state.screenConfiguration.preparedFinalObject,
        "SewerageConnection"
      );
...
const acknoledgementForWater = async (
  state,
  activeStep,
  isFormValid,
  dispatch
...
const acknoledgementForSewerage = async (
  state,
  activeStep,
  isFormValid,
  dispatch
...
export const changeStep = (
  ...
  if (activeStep === 0) {
    let conHolders = get(
      state,
      "screenConfiguration.preparedFinalObject.applyScreen.connectionHolders"
    );
    let isCheckedSameAsProperty =
      conHolders &&
      conHolders.length > 0 &&
      !conHolders[0].sameAsPropertyAddress
        ? false
        : true;
    dispatch(
      handleField(
...
export const renderSteps = (activeStep, dispatch) => {
  switch (activeStep) {
    case 0:
      ...
      break;
    case 1:
      let mStepSecond = isModifyMode()
        ? "formwizardThirdStep"
        : "formwizardSecondStep";
      dispatchMultipleFieldChangeAction(
        "apply",
        getActionDefinationForStepper(`components.div.children.${mStepSecond}`),
        dispatch
      );
      break;
    case 2:
      let mStep = isModifyMode()
        ? "formwizardSecondStep"
        : "formwizardThirdStep";
      dispatchMultipleFieldChangeAction(
        "apply",
        getActionDefinationForStepper(`components.div.children.${mStep}`),
        dispatch
      );
      break;
    default:
...
export const footerReview = (action, state, dispatch, status) => {
