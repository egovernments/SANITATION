...
export const workflowSearchCount = async (businessService) => {
  return httpRequest(
    `egov-workflow-v2/egov-wf/process/_count?tenantId=${localStorage.getItem(
      "inb-tenantId"
...
export const workflowSearch = async (businessService) => {
  return wfSearch([
    { key: "tenantId", value: localStorage.getItem("inb-tenantId") },
    { key: "offset", value: "0" },
    { key: "limit", value: "10" },
...
export const wfSearch = async (queryArg = []) => {
  return httpRequest(
    `egov-workflow-v2/egov-wf/process/_search?${convertQueryArgToString(
      queryArg
    )}`
  );
};
...
export const wfBusinessSearch = async (queryArg = []) => {
  return httpRequest(
    `egov-workflow-v2/egov-wf/businessservice/_search?${convertQueryArgToString(
      queryArg
    )}`
  );
};
...
export const getMdmsData = async (mdmsCriteria) => {
  return httpRequest(`mdms-v2/v1/_search`, { MdmsCriteria: mdmsCriteria });
};
...
export const wfEsclationSearch = async (queryArg = []) => {
  return httpRequest(
    `egov-workflow-v2/egov-wf/escalate/_search?${convertQueryArgToString(
      queryArg
    )}`
  );
};
