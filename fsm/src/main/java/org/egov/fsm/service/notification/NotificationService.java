...
//			List<String> payTriggerList = Arrays.asList(config.getPayTriggers().split("[,]"));
	...
	private void enrichSMSRequest(FSMRequest fsmRequest, List<SMSRequest> smsRequests) {
		...
		/*
		 * bug SAN-1088- Getting 2 sms for pay now application creation message code for
		 * pre_pay
		 */
		if (!FSMConstants.FSM_PAYMENT_PREFERENCE_POST_PAY.equalsIgnoreCase(fsm.getPaymentPreference())) {
			//FSM_ADVANCE_APP_CREATE
			/*
			 *  sms for pay now application creation message code for advance amount 
			 * pre_pay
			 */
			if (fsm.getAdvanceAmount() != null
				...
				log.info("messageCode  ::::FOR ADVANCE AMOUNT::::" + messageCode.toString());
			}
		...
		log.info("mobileNumberToOwner ::: {} "
				+ (null != mobileNumberToOwner ? mobileNumberToOwner.toString() : "null"));
		...
		if (fsmAddtlDtls != null && fsmAddtlDtls.get("payerMobileNumber") != null
				&& fsmAddtlDtls.get("payerName") != null
				&& mobileNumberToOwner.get(fsmAddtlDtls.get("payerMobileNumber")) == null) {
	...
	public void process(FSMRequest fsmRequest, FSM oldFSM) {
		...
		if (null != oldFSM.getVehicleCapacity() && null != newFSM.getVehicleCapacity()
			...
			/*
			 * Commenting out the SMS code as only notification is required for SAN-1024 if
			 * (null != config.getIsSMSEnabled()) { if (config.getIsSMSEnabled()) { if
